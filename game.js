...
! function(t, e, n, o, r) {
    ...
    function u(t, e) {
        t < 0 || t >= 3 || (e < 0 && (e = z[t] - 1), e >= z[t] && (e = 0), loginAvatarData[t] = e, s(n("#loginAvatar"), loginAvatarData, !1, 2), c())
    }
    ...
    function l() {
        if (st)
            if (rt && rt.drawingID == rt.myID) {
                if (ct.drawCommands.length > 0) {
                    if (ct.drawCommands.length > 8) {
                        var t = ct.drawCommands.splice(7);
                        st.emit("drawCommands", ct.drawCommands), ct.drawCommands = t
                    } else st.emit("drawCommands", ct.drawCommands), ct.drawCommands = []
                }
            } else ct.drawCommands.length > 0 && (ct.drawCommands = [])
    }
    ...
    function v() {
        "undefined" != typeof aipPlayer && (aiptag = aiptag || {}, aiptag.consented = !0, aiptag.cmd = aiptag.cmd || [], aiptag.cmd.display = aiptag.cmd.display || [], aiptag.cmd.player = aiptag.cmd.player || [], aiptag.cmd.player.push(function() {
    ...
    function w() {
        it.context && it.context.resume();
        var t = k(),
            e = io();
        e.emit("login", t), e.on("result", function(o) {
            o.code ? (n("#error-login").hide(), x(o.host, t)) : (n("#error-login").text(o.msg).show(), at.goto("login")), e.close()
        })
    }
    ...
    function k() {
        ...
        var o = "",
            r = t.location.href.split("?");
        r.length > 1 && (o = r[1]);
        var s = n("#loginLanguage").val();
        return {
            name: e[0],
            code: e[1],
    ...
    function x(t, e) {
        it.playSound("playerJoin"), st = io(t), st.on("connect", function() {
            st.on("disconnect", function() {
                st.close(), st = null, rt = null, at.goto("login"), n("#modalDisconnect").modal()
            }), st.emit("userData", e), st.on("kicked", function() {
                n("#modalKicked").modal()
            }), st.on("drawCommands", function(t) {
                for (var e = 0; e < t.length; e++) B(t[e])
            }), st.on("canvasClear", function() {
                ct.clear()
            }), st.on("lobbyKicked", function(t) {
                var e = rt.players.get(t);
                null != e && (rt.chatAddMsg(null, e.name + " was kicked!", j), e.kicked = !0)
            }), st.on("chat", function(t) {
                var e = rt.players.get(t.id),
                    n = rt.players.get(rt.myID),
                    o = e.id == rt.drawingID || e.guessedWord;
                n.id != rt.drawingID && !n.guessedWord && o || rt.chatAddMsg(e, b(t.message), o ? "#7dad3f" : "#000")
            }), st.on("lobbySpam", function() {
                rt.chatAddMsg(null, "Spam detected! You're sending too many messages.", j)
            }), st.on("restartNotification", function(t) {
                rt && rt.chatAddMsg(null, "Server will restart in about " + t + " seconds...", N)
            }), st.on("lobbyVotekickCurrent", function(t) {
                if (rt) {
                    var e = rt.players.get(t[0]),
                        n = rt.players.get(t[1]);
                    rt.chatAddMsg(null, "'" + e.name + "' is voting to kick '" + n.name + "' (" + t[2] + "/" + t[3] + ")", N)
                }
            }), st.on("lobbyConnected", function(t) {
                if (ct.clear(), E(""), rt = new Q(t), rt.inGame) {
                    for (var e = 0; e < t.drawCommands.length; e++) B(t.drawCommands[e]);
                    at.goto("game"), rt.drawingID >= 0 && A(rt.time)
                } else at.goto("lobby")
            }), st.on("lobbyLobby", function() {
                at.goto("lobby")
            }), st.on("lobbyState", function(t) {
                switch (parseInt(t[0])) {
                    case Y:
                        d(), A(rt.time);
                        break;
            ...
            }), st.on("lobbyDisconnected", function() {
                st.emit("lobbyLeave"), rt = null, at.goto("login")
            }), st.on("lobbyGameStart", function(t) {
                rt.reset(), E(""), at.goto("game")
            }), st.on("lobbyGameEnd", function() {
                var t = [];
                rt.players.forEach(function(e, n, o) {
                    t.push(e)
                }), I({
                    mode: "endgame",
                    text: "Result",
            ...
            }), st.on("lobbyLanguage", function(t) {
                rt.setLanguage(t)
            }), st.on("lobbyRounds", function(t) {
                rt.setRounds(t)
            }), st.on("lobbyDrawTime", function(t) {
                rt.setDrawTime(t)
            }), st.on("lobbyCustomWordsExclusive", function(t) {
                rt.setCustomWordsExclusive(t)
            }), st.on("lobbyPlayerConnected", function(t) {
                rt.addPlayer(t), rt.chatAddMsg(null, t.name + " joined.", O), it.playSound("playerJoin")
            }), st.on("lobbyPlayerDisconnected", function(t) {
                var e = rt.players.get(t);
                rt.removePlayer(e), e.kicked || rt.chatAddMsg(null, e.name + " left.", j), it.playSound("playerLeave")
            }), st.on("lobbyPlayerGuessedWord", function(t) {
                var e = rt.players.get(t);
                rt.setPlayerGuessed(e), rt.chatAddMsg(null, e.name + " guessed the word!", U), t == rt.myID && rt.chatDisable(), it.playSound("playerGuessed")
            }), st.on("lobbyGuessClose", function(t) {
                rt.chatAddMsg(null, "'" + t + "' is close!", N)
            }), st.on("lobbyPlayerDrawing", function(t) {
                var e = rt.players.get(t);
                rt.setPlayerDrawing(e.id), rt.chatAddMsg(null, e.name + " is drawing now!", _);
                var n = rt.drawingID == rt.myID;
                ct.setDrawing(n), n ? (f(), y()) : (p(), d()), it.playSound("roundStart"), A(rt.timeMax), R()
            }), st.on("lobbyOwnerChanged", function(t) {
                var e = rt.players.get(t);
                rt.setPlayerOwner(e), rt.chatAddMsg(null, e.name + " is the owner now!", U)
            }), st.on("lobbyRound", function(t) {
                rt.round = t, rt.updateRound(), I({
                    mode: "text",
                    text: "Round " + (rt.round + 1)
                })
            }), st.on("lobbyChooseWord", function(t) {
                if (t.id == rt.myID) I({
                    mode: "choosewords",
                    text: "Choose a word",
                    words: t.words
                    ...
                    I({
                        mode: "text",
                        text: rt.players.get(t.id).name + " is choosing a word!"
                    })
                }
            }), st.on("lobbyReveal", function(t) {
                C(), rt.chatEnable(), rt.chatAddMsg(null, "The word was '" + t.word + "'", U);
                for (var e = [], n = !0, o = 0; o < t.scores.length / 2; o++) {
                    ...
                    if (i) {
                        i.guessedWord && (n = !1);
                        var a = s - i.score;
                        rt.setPlayerScore(i, s, a), e.push(i)
                    }
                }
                rt.updateRanks(), I({
                    mode: "reveal",
                    text: "The word was: " + t.word,
                    revealReason: J[t.reason],
                    players: e,
                    onshow: function() {
                        n ? it.playSound("roundEndFailure") : it.playSound("roundEndSuccess")
                    }
                }), rt.setPlayerDrawing(-1), ct.setDrawing(!1), rt.playersResetGuessedWord()
            }), st.on("lobbyPlayerRateDrawing", function(t) {
                var e = rt.players.get(t[0]),
                    n = rt.players.get(t[1]),
                    o = t[2];
                e && rt.playerRate(e, o), n && (o > 0 ? n.thumbsUp++ : n.thumbsDown++)
            }), st.on("lobbyCurrentWord", function(t) {
                rt.players.get(rt.myID).guessedWord || E(t)
            }), st.on("lobbyTime", function(t) {
                D(t)
            })
        })
    }
    ...
    function C() {
        null != ft && (clearInterval(ft), ft = null)
    }

    function A(t) {
        D(t), C(), ft = setInterval(function() {
            dt > 0 && D(dt - 1)
        }, 1e3)
    }

    function D(t) {
        dt = t, dt <= 8 && it.playSound("tick"), n("#timer").text(dt)
    }

    function S() {
        at.goto("load"), st.emit("lobbyLeave")
    }
    ...
    function M(t, e, n) {
        if (rt.checkDrawing() && (ct.updateMousePosition(t, e, n), ct.brush.down)) {
            var o = null;
            switch (ct.brush.tool) {
                case "pen":
                    o = ct.createDrawCommandLine(ct.brush.colorIndex, ct.brush.thickness, ct.mouseposPrev.x, ct.mouseposPrev.y, ct.mousepos.x, ct.mousepos.y);
                    break;
                case "erase":
                    o = ct.createDrawCommandErase(ct.brush.thickness, ct.mouseposPrev.x, ct.mouseposPrev.y, ct.mousepos.x, ct.mousepos.y);
                    break;
                case "fill":
    ...
    function B(t) {
        rt.drawingID == rt.myID ? (ct.drawCommands.push(t), ct.performDrawCommand(t)) : ct.addDrawCommandReceived(t)
    }

    function I(t) {
        if (ht) return ut.unshift(t), void R();
        ht = !0;
        var e = n("#overlay");
        e.show(), e.animate({
            opacity: "1"
    ...
    function L(t) {
        ...
        if ("choosewords" == t.mode) {
            ...
            for (var i = 0; i < t.words.length; i++) {
                ...
                a.data("id", i), a.text(t.words[i]), r.append(a), a.on("click", function() {
                    ...
                    st && st.emit("lobbyChooseWord", t), R()
                })
            }
        } else r.hide();
        ...
        if ("endgame" == t.mode) {
            ...
            for (var i = 0; i < t.players.length; i++) {
                var h = t.players[i],
                    l = T(n("#gameEndPlayerDummy"));
                l.find(".name").text(h.name);
                var g = l.find(".rank");
                g.text("#" + h.rank), h.rank <= 3 && g.addClass("rank-" + h.rank), h.rank <= 3 ? y.append(l) : m.append(l), s(l, h.avatar, h.id == rt.ownerID, h.rank <= 3 ? 2 : 1)
            }
        } else d.hide();
        o.show(), o.animate({
            bottom: "0%"
        }, G, "easeInOutBack", function() {
            t.onshow && t.onshow()
        })
    }
                        ...
                        if (!n("json")) {
                            ...
                            if ((d = g.hasOwnProperty) || (d = function(t) {
                                ...
                                var E = {
                                    ...
                                    B = function(t, e, n, o, r, s, i) {
                                        var a, c, u, h, p, f, g, b, w, A, S, E, I, L, R, U;
                                        try {
                                            a = e[t]
                                        } catch (t) {}
                                        if ("object" == typeof a && a)
                                            ...
                                            a = (u <= 0 || u >= 1e4 ? (u < 0 ? "-" : "+") + M(6, u < 0 ? -u : u) : M(4, u)) + "-" + M(2, h + 1) + "-" + M(2, p) + "T" + M(2, g) + ":" + M(2, b) + ":" + M(2, w) + "." + M(3, A) + "Z"
                                        } else a = null;
                                        ...
                                        if ("object" == typeof a) {
                                            ...
                                            if (i.push(a), S = [], R = s, s += r, c == C) {
                                                for (I = 0, L = a.length; I < L; I++) E = B(I, a, n, o, r, s, i), S.push(E === m ? "null" : E);
                                                U = S.length ? r ? "[\n" + s + S.join(",\n" + s) + "\n" + R + "]" : "[" + S.join(",") + "]" : "[]"
                            ...
                            if (!n("json-parse")) {
                                var I, L, R = r.fromCharCode,
                                        ...
                                        for (var t, e, n, o, r, s = L, i = s.length; I < i;) switch (r = s.charCodeAt(I)) {
                                            ...
                                            case 34:
                                                for (t = "@", I++; I < i;)
                                                    ...
                                                    case 114:
                                                        ...
                                                        break;
                                                    case 117:
                                                        for (e = ++I, n = I + 4; I < n; I++)(r = s.charCodeAt(I)) >= 48 && r <= 57 || r >= 97 && r <= 102 || r >= 65 && r <= 70 || N();
                                                        t += R("0x" + s.slice(e, I));
                                                        break;
                                                    default:
                                                        N()
                                            ...
                                            default:
                                                if (e = I, 45 == r && (o = !0, r = s.charCodeAt(++I)), r >= 48 && r <= 57) {
                                                    for (48 == r && (r = s.charCodeAt(I + 1)) >= 48 && r <= 57 && N(), o = !1; I < i && (r = s.charCodeAt(I)) >= 48 && r <= 57; I++);
                                                ...
                                                if (o && N(), "true" == s.slice(I, I + 4)) return I += 4, !0;
                                    ...
                                    O = function(t) {
                                        ...
                                        if ("$" == t && N(), "string" == typeof t) {
                                            ...
                                            if ("[" == t) {
                                                for (e = [];
                                                    "]" != (t = _()); n || (n = !0)) n && ("," == t ? "]" == (t = _()) && N() : N()), "," == t && N(), e.push(O(t));
                                                return e
                                            }
                                            if ("{" == t) {
                                                for (e = {};
                                                    "}" != (t = _()); n || (n = !0)) n && ("," == t ? "}" == (t = _()) && N() : N()), "," != t && "string" == typeof t && "@" == (A ? t.charAt(0) : t[0]) && ":" == _() || N(), e[t.slice(1)] = O(_());
                                                return e
                                            }
                                            N()
                                        }
                                ...
                                e.parse = function(t, e) {
                                    var n, o;
                                    return I = 0, L = "" + t, n = O(_()), "$" != _() && N(), I = L = null, e && v.call(e) == w ? G((o = {}, o[""] = n, o), "", e) : n
                                }
                            }
                        }
    ...
    K.prototype.setMute = function(t) {
    ...
    }, K.prototype.addSound = function(t, e) {
        this.sounds.set(e, new $(this, t))
    }, K.prototype.loadSounds = function() {
    ...
    }, K.prototype.playSound = function(t) {
    ...
    var V = function(t) {
        this.canvas = t, this.canvasCtx = this.canvas[0].getContext("2d"), this.canvasCtx.mozImageSmoothingEnabled = !1, this.canvasCtx.webkitImageSmoothingEnabled = !1, this.canvasCtx.msImageSmoothingEnabled = !1, this.canvasCtx.imageSmoothingEnabled = !1, this.mouseposPrev = {
            ...
            x: 0,
            y: 0
        }, this.drawCommands = [], this.drawCommandsReceived = [], this.brush = new Z;
        var e = this;
        setInterval(function() {
            e.drawCommandsReceived.length > 0 && e.performDrawCommand(e.drawCommandsReceived.shift())
        }, 3), this.clear()
    };
    V.prototype.updateMousePosition = function(t, e, n) {
    ...
    }, V.prototype.addDrawCommandReceived = function(t) {
        this.drawCommandsReceived.push(t)
    }, V.prototype.addDrawCommand = function(t) {
        this.drawCommands.push(t)
    }, V.prototype.createDrawCommandLine = function(t, e, n, o, r, s) {
        return [0, t, e, n, o, r, s]
    }, V.prototype.createDrawCommandErase = function(t, e, n, o, r) {
        return [1, t, e, n, o, r]
    }, V.prototype.createDrawCommandFill = function(t, e, n) {
        return [2, t, e, n]
    }, V.prototype.performDrawCommand = function(t) {
        switch (t[0]) {
            case 0:
    ...
    }, V.prototype.plotLineOld = function(t, e, n, o, r, s) {
    ...
    }, V.prototype.plotLine = function(t, e, n, o, r, s, i, a) {
    ...
    }, Z.prototype.updateBrushCursor = function() {
        switch (this.tool) {
            case "pen":
            case "erase":
    ...
    Q.prototype.updatePlayerAvatars = function() {
        var t = this;
    ...
    }, Q.prototype.containerGamePlayerClear = function() {
        this.containerGame.empty()
    }, Q.prototype.containerGamePlayerGet = function(t) {
        return this.containerGame.find("#player" + t.id)
    }, Q.prototype.containerGamePlayerAdd = function(t) {
    ...
    }, Q.prototype.containerLobbyPlayerRemove = function(t) {
        this.containerLobbyPlayerGet(t).remove()
    }, Q.prototype.containerLobbyPlayersUpdateOwner = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    }, Q.prototype.containerLobbyPlayerClear = function() {
        this.containerLobby.empty()
    }, Q.prototype.containerLobbyUpdateRounds = function() {
        n("#lobbySetRounds").val(this.roundMax)
    }, Q.prototype.containerLobbyUpdateDrawTime = function() {
        n("#lobbySetDrawTime").val(this.timeMax)
    }, Q.prototype.containerLobbyUpdateUseCustomWordsExclusive = function() {
        n("#lobbyCustomWordsExclusive").attr("checked", !!this.useCustomWordsExclusive)
    }, Q.prototype.containerLobbyUpdateLanguage = function() {
        var t = P(this.language);
        n("#lobbySetLanguage").val(t)
    }, Q.prototype.playerMessage = function(t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        n.find(".text").show(), n.find(".text").text(e)
    }, Q.prototype.playerRate = function(t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        e > 0 ? n.find(".iconThumbsUp").show() : n.find(".iconThumbsDown").show()
    }, Q.prototype.playerMessageHandle = function(t) {
    ...
    }, Q.prototype.chatClear = function() {
        n("#boxMessages").empty()
    }, Q.prototype.chatAddMsg = function(t, e, o) {
        var r = n("#boxMessages"),
    ...
    }, Q.prototype.chatDisable = function() {}, Q.prototype.chatEnable = function() {};
    var tt = function(t, e) {
    ...
    var et = function() {
        this.screens = {
            load: new tt(n("#screenLoading"), 0),
            login: new tt(n("#screenLogin"), 2),
            lobby: new tt(n("#screenLobby"), 1),
            game: new tt(n("#screenGame"), 1)
        }, this.screenCurrent = null
    };
    et.prototype.goto = function(t) {
    ...
    }, et.prototype.hideLogo = function() {
        n("#containerLogoBig").hide(), n("#containerLogoSmall").hide()
    }, et.prototype.showLogo = function(t) {
    ...
    var nt = t.location != t.parent.location ? e.referrer : e.location.href,
        ot = function(t, e) {
            return -1 !== t.indexOf(e)
        };
    (ot(nt, "skribbl-io.com") || ot(nt, "skribblio.com") || ot(nt, "skribbl-io.org")) && (t.top.location.href = "https://www.skribbl.io"), n(function() {
    ...
    var rt = null,
        ...
        it = new K,
        at = new et,
        ct = new V(n("#canvasGame")),
        ut = new Array,
        ht = !1,
        lt = !1;
    ct.setDrawing(!1), at.goto("login");
    var pt = !1;
    at.screens.game.onshow = function() {
            pt || (pt = !0, (adsbygoogle = t.adsbygoogle || []).push({}))
        ...
        function() {
            for (var t = n("#logoAvatarDummy"), e = n("#logoAvatarContainer"), o = Math.round(8 * Math.random()), r = 0; r < 8; r++) {
                ...
                e.append(i);
                var a = [0, 0, 0, 0];
                a[0] = r, a[1] = Math.round(100 * Math.random()) % H, a[2] = Math.round(100 * Math.random()) % F, a[3] = 1e3 * Math.random() < 70 ? Math.round(1 * Math.random()) : -1, s(i, a, r == o, 1)
            }
            ...
            return t ? t = JSON.parse(t) : (t = [Math.round(Math.random() * z[0]), Math.round(Math.random() * z[1]), Math.round(Math.random() * z[2]), -1], o.setItem("avatar", JSON.stringify(t))), t
        }(), n("#buttonAvatarCustomizerRandomize").on("click", function() {
            return loginAvatarData[0] = Math.floor(Math.random() * z[0]), loginAvatarData[1] = Math.floor(Math.random() * z[1]), loginAvatarData[2] = Math.floor(Math.random() * z[2]), c(), s(n("#loginAvatar"), loginAvatarData, !1, 2), !1
            ...
            function o() {
                if (rt) {
                    var e = rt.checkDrawing() ? 90 : 190;
                    r++, r == e && h(), r >= e + 7 && t.location.reload(!0)
                }
            }
            var r = 0;
            setInterval(o, 1e3);
            ...
            st && st.emit("rateDrawing", 1), f()
        }), n("#rateDrawing .thumbsDown").on("click", function() {
            st && st.emit("rateDrawing", 0), f()
        }), f(), y(), n("#votekickCurrentplayer").on("click", m), n("#formLogin").submit(function(t) {
            t.preventDefault(), lt = !1, g()
        }), n("#buttonLoginCreatePrivate").on("click", function() {
            lt = !0, g()
        }),
        function(t, n) {
            var o = e.head || e.getElementsByTagName("head")[0],
                ...
                s = !0;
            r.async = "async", r.type = "text/javascript", r.charset = "UTF-8", r.src = t, r.onload = r.onreadystatechange = function() {
                !s || r.readyState && !/loaded|complete/.test(r.readyState) || (s = !1, n(), r.onload = r.onreadystatechange = null)
            }, o.appendChild(r)
        }("//api.adinplay.com/libs/aiptag/pub/SKR/skribbl.io/tag.min.js", v);
    var ft = null,
        dt = 0;
    D(80), n("#lobbySetLanguage").on("change", function() {
        st && rt.myID == rt.ownerID && st.emit("lobbySetLanguage", n(this).val())
    }), n("#lobbySetRounds").on("change", function() {
        st && rt.myID == rt.ownerID && st.emit("lobbySetRounds", n(this).val())
    }), n("#lobbySetDrawTime").on("change", function() {
        st && rt.myID == rt.ownerID && st.emit("lobbySetDrawTime", n(this).val())
    }), n("#lobbyCustomWordsExclusive:checkbox").change(function() {
        if (st && rt.myID == rt.ownerID) {
            var t = this.checked;
            st.emit("lobbySetCustomWordsExclusive", t)
        }
        ...
        at.goto("load"), st.emit("joinRandom")
    }), n("#buttonLobbyLeave").on("click", S), n("#buttonGameLeave").on("click", S), n("#buttonLobbyPlay").on("click", function() {
        st && rt.myID == rt.ownerID && st.emit("lobbyGameStart", n("#lobbySetCustomWords").val())
    }), n("#formChat").submit(function(t) {
        var e = n("#inputChat"),
            o = e.val();
        return o && (st ? st.emit("chat", o) : rt.chatAddMsg(rt.players.get(rt.myID), o, "#000"), e.val("")), !1
        ...
        st.emit("lobbyCreate", t), n("#modalCreateLobby").modal("hide")
    }), n("#buttonRefreshLobbies").on("click", function() {
        st.emit("requestLobbies")
    }), ct.canvas.on("mousewheel DOMMouseScroll", function(t) {
        t.preventDefault();
        var e = t.originalEvent.wheelDelta > 0 || t.originalEvent.detail < 0 ? 1 : -1;
        ct.brush.setThickness(ct.brush.thickness + 6 * e)
    }), n(e).on("mousemove", function(t) {
        rt && rt.checkDrawing() && M(t.clientX, t.clientY, !1)
    }), n(e).keypress(function(t) {
        if (rt && rt.checkDrawing() && t !== r && t.key !== r) switch (t.key.toUpperCase()) {
            case "B":
        ...
        switch (t.preventDefault(), t.button) {
            case 0:
                ct.brush.down || (ct.brush.setDown(!0), M(t.clientX, t.clientY, !0))
        }
    });
    var yt = null;
    ct.canvas.on("touchstart", function(t) {
        t.preventDefault();
        var e = t.changedTouches;
        e.length > 0 && null == yt && (yt = e[0].identitfier, ct.brush.setDown(!0), M(e[0].clientX, e[0].clientY, !0))
    }), ct.canvas.on("touchend", function(t) {
        t.preventDefault();
        var e = t.changedTouches;
        if (e.length > 0 && null != yt)
            for (var n = 0; n < e.length; n++)
                if (e[n].identitfier == yt) {
                    ct.brush.setDown(!1), yt = null;
                    break
                }
    }), ct.canvas.on("touchcancel", function(t) {
        t.preventDefault();
    ...
    }), ct.canvas.on("touchmove", function(t) {
        t.preventDefault();
        for (var e = t.changedTouches, n = 0; n < e.length; n++)
            if (e[n].identitfier == yt) {
                M(e[n].clientX, e[n].clientY, !1);
                break
            }
        ...
        st ? st.emit("canvasClear") : ct.clear()
    }), n(".containerTools .tool").on("click", function() {
        ct.brush.setTool(n(this).data("tool"))
    }), n(".colorItem").on("click", function() {
        var t = n(this).data("color");
        ct.brush.setColor(Number(t))
    }), n(".brushSize").on("click", function() {
        var t = ct.brush.thicknessMin,
            e = ct.brush.thicknessMax,
            o = Number(n(this).data("size")) * (e - t) + t;
        ct.brush.setThickness(o)
    })
